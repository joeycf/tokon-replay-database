 ...
 * EVERY RUN. The witness file is rebuilt from scratch by each pull and holds
 * only the pages that pull read, so a cursor morning's window is the front page
 * or two of the feed and its numbers differ from yesterday's — a different
 * WINDOW, not a different corpus. Rendering those into report.md made the file
 * change every single morning whether or not any RECORD had, which defeats the
 * cron's no-change-no-commit rule from the other side and puts a deploy on the
 * calendar every day forever. That is the same failure the `_Generated`
 * timestamp already has a suppression rule for, arriving through a new door.
 *
...
export interface WitnessArtifact {
  /** The reading, frozen at the last full sweep. Absent until one has run. */
  measured?: {
 ...
 * which is what keeps a quiet morning quiet.
 */
export function formatCrossCheck(a: WitnessArtifact): string[] {
  ...
  return [
    ...
    '',
    // "video(s)", not "entr(ies)": the loop that fills `unmatched` runs over
    // byVideo KEYS and increments once per key, so this counts distinct videos
    // the catalogue points at, not catalogue rows — several of which can name
    // the same video. The JSDoc above already said videos; this line said
    // entries, and the file contradicted itself.
    `_Measured on the last full sweep, at catalogue entry ${m.atEntryId}. ${m.unmatched} catalogue video(s) point at videos_`,
    `_we do not hold; ${m.segmented} are VODs the catalogue segments, which the index intake owns._`,
    '',
